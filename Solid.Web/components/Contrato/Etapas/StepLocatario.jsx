...
const StepLocatario = ({ handleTipoLocatario }) => {
    ...
    const handlePFPick = () => {
        setIsPFPicked(true);
        setIsPJPicked(false);
        handleTipoLocatario(TipoPessoa.Fisica);
    };

    const handlePJPick = () => {
        setIsPFPicked(false);
        setIsPJPicked(true);
        handleTipoLocatario(TipoPessoa.Juridica);
    };

    return (
        <>
            <label className="text-2xl sm:text-3xl font-medium text-black-600 leading-normal mx-auto">
            Quem vai alugar o imóvel (Locatário), é uma pessoa física ou jurídica?
            </label>
